...
const InstructorExamPage = () => {
  ...
  const handleNumQuestionsChange = (e) => {
    ...
    if (newNumQuestions < 1) {
      ...
      const newExamData = Array.from({ length: newNumQuestions }, () => ({
        question: "",
        options: ["", "", "", ""],
        correct_answer: "",
      }));
        ...
        querySnapshot.forEach((doc) => {
          ...
          if (examData.startTime) {
            ...
            if (startTime >= currentDate) {
              // Only include exams from the current date onwards
  ...
  const handleSubmit = async () => {
    ...
    const durationText = `${hoursText}${
      hoursText && minsText ? " " : ""
    }${minsText}`;

    const startTimeTime = new Date(`${startTime}T${examTime}`);
    const endTime = new Date(
      startTimeTime.getTime() + hours * 60 * 60 * 1000 + minutes * 60 * 1000
    );
        ...
        questions.map(async (question, index) => {
          ...
          if (question.id) {
            ...
            const newQuestionRef = await addDoc(
              ...
              newQuestionFirestore
            );
            console.log(
              "New question added to Firestore with ID:",
              newQuestionRef.id
            );
          }
        })
    ...
    if (confirmation) {
      try {
        ...
        const examsSnapshot = await getDocs(
          query(collection(db, "exams"), where("examId", "==", examId))
        );
        ...
        <InstructorDashboardContainer>
          ...
          {activeContent === "examForm" && (
            <div>
              ...
              <SetExamTableContainer>
                ...
                <SetExamTableRow>
                  ...
                  <QuestionInput
                    ...
                    value={examDuration}
                    onChange={(e) =>
                      setExamDuration(formatExamDuration(e.target.value))
                    }
                    placeholder="HH:MM"
                    required
                    style={{ width: "80px" }}
              ...
              <button onClick={handleAddQuestion} style={{ width: "150px" }}>
                Set Questions
              </button>
                    ...
                    upcomingExams.map((exam) => (
                      <div key={exam.id}>
                        <ExamDescription>
                          ...
                          >
                            {formatDateInViewExams(exam.startTime)}
                          </span>
                          <br />
                          Time: {exam.startTime.toDate().toLocaleTimeString()}
                          <br />
                          Number of Questions:{" "}
                          <span
                            style={{
                    ...
                    modalContent={
                      <>
                        {editMode
                          ...
                          : formatQuestionsForModal(questions)}
                        {renderActionButtons()}
                      </>
